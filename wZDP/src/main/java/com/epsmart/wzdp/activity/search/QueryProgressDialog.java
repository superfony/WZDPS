...
public class QueryProgressDialog {
	private static final String TAG = QueryProgressDialog.class.getName();
	...
	public QueryProgressDialog() {
		super();
	}

	public QueryProgressDialog(final Activity activity,
		...
		popView = layoutInflater.inflate(R.layout.contract_query, null);
		...
		query.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				req = RequestXmlHelp.getCommonXML(RequestXmlHelp.getReqXML(
						"crdate", et_one.getText().toString())
						.append(RequestXmlHelp.getReqXML("pspid",((CommonReqAct) activity).getPspid())));
	...
	private void onSuccessNew(BasicResponseNew response) {
		...
		if (null == response.basicEntityList
			...
			return;
		String ver = response.basicEntityList.get(0).fields.get("crdate").fieldContent;
		crdate = ver.split(",");

		AlertDialog.Builder builder = new Builder(mActivity);
		builder.setTitle("选择填报日期")
				.setCancelable(true)
				.setSingleChoiceItems(crdate, -1,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog,
									int which) {
								dialog.cancel();
								et_one.setText(crdate[which]);
