 ...
 * 项目施工节点信息表单
 */
public class ProInfoFragment extends SceneFragmemt{
	...
	private String TAG = ProInfoFragment.class.getName();
	...
	public void loadData(RequestPram param) {
		...
		requestPram.methodName=RequestParamConfig.projectQueryReq;
...
//				new ProcessResponse(), RequestType.THRIFT);
	...
	protected void submitMethod(RequestPram requestPram) {
		...
		requestPram.pluginId=119;
		requestPram.userName=appContext.user.getUid();
		requestPram.methodName =RequestParamConfig.projectNodeUpload;
		requestPram.param = fillHelpNew.getparams(getArguments().getString("reqPs"));
