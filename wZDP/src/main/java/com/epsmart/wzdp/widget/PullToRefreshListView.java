package com.epsmart.wzdp.widget;
...
public class PullToRefreshListView extends ListView implements OnScrollListener {  
 ...
 // 这对PullToRefreshListView与ExtendedListView 进行整合
 ...
 public PullToRefreshListView(Context context, AttributeSet attrs) {  
     super(context, attrs);  
     init(context);  
		final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullBarListView);
		final int scrollBarPanelLayoutId = a.getResourceId(R.styleable.PullBarListView_scrollBarPanel, -1);
		final int scrollBarPanelInAnimation = a.getResourceId(R.styleable.PullBarListView_scrollBarPanelInAnimation, R.anim.in_animation);
		final int scrollBarPanelOutAnimation = a.getResourceId(R.styleable.PullBarListView_scrollBarPanelOutAnimation, R.anim.out_animation);
		a.recycle();
 ...
 public void onScroll(AbsListView view, int firstVisiableItem, int visibleItemCount,  int totalItemCount) {  
     ...
     //add
		if (null != mPositionChangedListener && null != mScrollBarPanel) {
			if (totalItemCount > 0) {
				...
				for (int i = 0; i < count; ++i) {
					...
					if (childView != null) {
						if (thumbOffset > childView.getTop() && thumbOffset < childView.getBottom()) {
							if (mLastPosition != firstVisiableItem + i) {
								mLastPosition = firstVisiableItem + i;
				...
				mScrollBarPanelPosition = thumbOffset - mScrollBarPanel.getMeasuredHeight() / 2;
				 Log.i(TAG, "mScrollBarPanelPosition="+mScrollBarPanelPosition);
		...
		// Log.i(TAG, "mOnScrollListener="+mOnScrollListener);
...
//		}
 ...
 private void changeHeaderViewByState() {  
     ...
     case DONE:  
         ...
         arrowImageView.clearAnimation();  
         // 此处更换图标   
         arrowImageView.setImageResource(R.drawable.ic_pulltorefresh_arrows);  
 ...
 /******************PullToRefreshListView with bar  start*******************/
