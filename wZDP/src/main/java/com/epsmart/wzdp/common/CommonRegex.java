...
public class CommonRegex {
	...
	/**
	 * 从origin字符串中获取tag标签内的值,形如&lt;result>0&lt;/result>
	 * 
	 * @param tag
	 * @param origin
	 * @return
	 */
	public static String getMiddleValue(String tag, String origin) {
		try {
	...
	public static String getFormatNumber(String priceStr) {
		if (null == priceStr)
			return priceStr;
		String price = null;
		double doublePrice;
		if (priceStr.matches("^\\s*\\d+\\s*$")) {
			doublePrice = Double.valueOf(priceStr) / 100;
			price = NumberFormat.getCurrencyInstance(Locale.CHINA).format(
					doublePrice);
		} else {
			price = priceStr;
		}
		return price.replace("CN", "");
	}
}
