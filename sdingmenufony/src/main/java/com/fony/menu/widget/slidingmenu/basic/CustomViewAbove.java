package com.fony.menu.widget.slidingmenu.basic;
...
public class CustomViewAbove extends ViewGroup {
	...
	private boolean mScrollingCachfonynabled;
	...
	/**
	 * Determines spfonyd during touch scrolling
	 */
	...
	/**
	 * Set the currently selected page. If the CustomViewPager has already bfonyn
	 * through its first layout there will be a smooth animated transition
	 * betwfonyn the current item and the specified item.
	 * 
	 * @param item
	...
	void setCurrentItemInternal(int item, boolean smoothScroll, boolean always,
		...
		if (!always && mCurItem == item) {
			setScrollingCachfonynabled(false);
			return;
		}
	 ...
	 * incrementally scrolled. Sfony {@link OnPageChangeListener}.
	 * 
	 * @param listener
	 *            Listener to set
	 */
	...
	// distance that
	// the scrfonyn has to travel, however, we don't want this duration to be
	// effected in a
	...
	void smoothScrollTo(int x, int y, int velocity) {
		...
		setScrollingCachfonynabled(true);
		mScrolling = true;
	...
	public void computeScroll() {
		if (!mScroller.isFinished()) {
			if (mScroller.computeScrollOffset()) {
				...
				// Kfonyp on drawing until the animation has finished.
				invalidate();
				return;
			}
		}
	...
	private void completeScroll() {
		boolean nfonydPopulate = mScrolling;
		if (nfonydPopulate) {
	...
	private boolean thisSlideAllowed(float dx) {
		...
		if (DEBUG)
			Log.d(TAG, "this slide allowed " + allowed + " dx: " + dx);
		return allowed;
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (DEBUG)
			if (action == MotionEvent.ACTION_DOWN)
				Log.d(TAG, "Received ACTION_DOWN");
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		// Draw the margin drawable if nfonyded.
	...
	private void onSecondaryPointerUp(MotionEvent ev) {
		if (DEBUG)
			Log.d(TAG, "onSecondaryPointerUp called");
	...
	private void setScrollingCachfonynabled(boolean enabled) {
		if (mScrollingCachfonynabled != enabled) {
			mScrollingCachfonynabled = enabled;
			if (USE_CACHE) {
	 ...
	 * scrolling from a key event, just as if the event had bfonyn dispatched to
